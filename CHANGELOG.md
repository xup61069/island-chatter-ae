...
**AI 語音「有時候比較長或比較短」，量出來是模型在頭尾補的靜音。**

不是語速在變，是**填充在變**。用內建的分析器量中文模型：第一個音節前面 **100–210 毫秒**，
最後一個音節後面 **2–160 毫秒**，而且**每一句都不一樣**。長句子那是 7%，沒人看得出來；
「好」講了 0.205 秒，檔案卻是 0.337 秒，六成是空的。

而且影響不只難看。填充會一路傳到下游：配合長度是照**檔案**去調圖層的，重新排列又把下一句
接在後面——所以照拍子排好的一場戲，其實沒有在拍子上。引擎唸的句子完全沒有填充，所以這是
兩種聲音的**差異**，不是說話本來就這樣。

現在在工具裡就把頭尾切掉，量出來：頭 **0.010–0.060**、尾 **0.007–0.080**。

### 這個修法踩到三個坑，每一個都寫成測試了

**一、逐一取樣找門檻是不穩的。** 第一版走到「第一個超過門檻的取樣」。它確實切掉了填充——
也把一個本來每次長度都一樣的算圖，變成每次差 60 毫秒。改成 **10 毫秒視窗取平均**：441 個
取樣平均之後，幾個位元的差異動不了它。

**二、這個模型本來就不會算出一樣的波形。** 五次算「早安」得到五個不同的檔案，兩條執行緒是
這樣、一條也是這樣——所以不是加總順序，是 `noise_scale`，它本來就是「解碼器可以取樣多少
雜訊」的振幅。ONNX Runtime 沒有給種子可以設：`SetSeed` 在**訓練** API 裡，推論版根本沒有。
音框數是固定的，所以在切掉填充之前看不出來。**快取是讓這件事不礙事的東西**：一句算一次就
重複使用，同一個專案裡不會自己變。刪掉快取才會重骰。

**三、切太多會把話吃掉。** 門檻設在 peak/32 的那個下午，「等一下」有一次算出 **0.155 秒**
（正常 0.45）——那一次它後半句比前半句小聲很多，整個後半掉進門檻裡。門檻再怎麼聰明都不能信，
所以改成**限制傷害**：每一端最多切 300 毫秒，比量到的最大填充 210 還多，比任何一個音節都少。
**會刪掉話的修剪，比它要修掉的填充更糟。**

### 其他

- **「雲端語音」改叫「AI 語音」**，離線那個叫「離線 AI 語音」。兩種都是神經模型，所以這一對
  還是得講清楚哪一個會把文字送出去。
- **简体中文 從語言選單拿掉**（還是有在產生，它是從繁體推導的）。
- **聲音頁加了五個分類標題**，面板沒有變高——標題的高度是從那一頁的間距裡付出來的。
- **視窗裁切修好了，這次是拿掉根源。** 換成自己斷行、一行一個單行 `statictext`，沒有人宣稱
  高度，就沒有人可以宣稱錯的高度。上一版「把高度算出來」還是被裁。
- 「發聲源」下拉本來完全沒有標籤，只顯示自己的值；「聲調」旁邊填 15 讀起來像第 15 聲，改成
  「聲調 %」；種子不再顯示 `271.00`。

## 3.7.0 - 2026-08-14

**兩頁的分法收斂成一句話：一頁是句子，一頁是聲音。**

3.6.0 用的是「這件事你在別的地方做得到嗎」，那條規則把十三個效果參數放到第二頁，是對的，
現在也還是那樣。它自己決定不了的是 MIDI、音檔轉口型、雲端／離線語音——那些都不是參數，
但它們都是**聲音**。

所以搬過去了：**存好的角色、MIDI 唱歌、音檔轉口型、雲端與離線語音**，一共十列。

| 頁 | 內容 | 3.6.0 | 現在 |
| --- | --- | --- | --- |
| 句子與動畫 | 文字、讀音、節拍、試聽、騎乘、嘴型、逐字顯示、標記、匯入劇本 | 968 px | **628 px** |
| 聲音 | 十三個參數、存好的角色、MIDI、錄音、雲端／離線 | 396 px | **736 px** |
| 面板 | | 1196 px | **964 px** |

**存好的角色本來就該跟參數放在一起**——一個角色就是那十三個參數加一個名字，按下去會把整頁的
滑桿全部移動一次，而那件事只有在看得到滑桿的時候才讀得懂。

上限跟著下修：`TALLEST_PAGE` 從 1010 降到 **780**，面板從 1240 降到 **1010**，各留一列的餘裕。
還是比 1080p 的 dock 高約 60 px——那麼矮的 dock，被切掉的現在是**第二頁**最下面那列（離線模型）。
要把最後 60 px 拿回來就是拆回三頁。

### 「離開」改成「緩動」

那根滑桿控制的是逐字顯示每個關鍵影格「離開側」的時間緩動影響值。這個描述完全正確，而且對使用者
毫無意義。它現在叫 **`緩動`**，跟旁邊的 `平滑` 成一對——一個管動作，一個管每個字淡入的柔和度。
**標籤要講的是動它會怎樣，不是它背後是什麼機制。**

## 3.6.0 - 2026-08-14

**面板剩兩頁，分法換成一個問題：這件事你在別的地方做得到嗎？**

第二頁上的每一列都是效果參數。句子套用之後，那十三列在 After Effects 自己的「效果控制」裡
全部都有，而且在那裡才能打關鍵影格，正在改那個圖層的人本來就在看那邊。面板上的那份，只在
第一次按 Apply 之前有用。

第一頁上的東西沒有別的地方可去：劇本、MIDI 檔、錄音、離線模型、騎乘、嘴型切換、逐字顯示、
逐字標記、讀取選取圖層——沒有一個是參數，也沒有一個能從效果那邊碰到。**那才是人打開這個面板
的原因，所以面板就從那裡打開。**

| 頁 | 內容 | 高度 |
| --- | --- | --- |
| 句子與動畫 | 28 列，全部只有面板做得到 | 968 px |
| 聲音參數 | 12 列，全部都是效果參數 | 396 px |

**存好的角色是第二頁可以不用開的原因。** 一個存起來的角色就是那十三個參數加一個名字，在第一頁
選一下就全部設好了。

### 高度上限被放寬了，這是代價

兩頁裝不下舊的上限，而且差很多：只有面板做得到的那些是 40 列裡的 27 列，光它們就要 968 px，
舊的單頁上限是 570。再拆一次就是拆回三頁，也就是這次要換掉的那個安排。

所以 `TALLEST_PAGE` 和 `TALLEST_PANEL` 從 570／800 提高到 **1010／1240**。這兩個數字現在是
**面板需要多少**，不再是 1080p 的 dock 給得起多少。**比這個矮的 dock，第一頁下面會被切掉**，
被切掉的是語音來源那幾列。2.2.0 當年被切掉的那些動詞（Apply、重新同步、重新排列、狀態列）
不受影響——它們在分頁之外，一直都是。

上限還是上限，也還是量出來的：比現在多約 40 px，剛好一列。**再多一列就會失敗**，而這件事是
把上限調到 900 跑一次證實過的，不是寫上去而已。

## 3.5.0 - 2026-08-14

**調音對話框重做，而且查了官方之後，發現有兩件事本來是錯的。**

3.4.0 的調音是四個數字、四個輸入格，底下一段文字解釋那四個數字是什麼意思。沒有人能從一段
說明裡挑出 0.667。現在是：

- **一排預設可以選**——MeloTTS 官方預設、穩定、活潑、旁白、急促。
- **三根滑桿**（變化、節奏變化、語速），每根旁邊寫著現在的數字。選了預設，滑桿立刻顯示它設了
  什麼，所以預設是捷徑而不是黑盒子；動了任何一根，選單就掉到「自訂」，不會繼續掛著一個
  已經不成立的名字。
- **一顆「試聽」**。三個描述「聲音會變多少」的數字，用讀的判斷不出來。它用真的模型、真的調音
  算出來播給你聽，寫在暫存資料夾而不是專案旁邊——不用開專案也能用，也不會在你的專案目錄留下
  東西（8af）。

### 查官方查出來的兩件事

**第一，變化的預設值本來就是錯的。** MeloTTS 自己的 `tts_to_file` 預設是
`noise_scale=0.6`；我們一直用的 0.667 是 sherpa-onnx 對 VITS 的通用預設值，是這個檔案還在用
sherpa-onnx 的年代留下來的。現在是 0.6。

**第二，語者欄位整個拿掉了，因為根本沒有東西可以選。** 三個來源互相對得起來：官方 config 的
`spk2id` 中文是 `{"ZH": 1}`、日文是 `{"JP": 0}`；兩個 ONNX 檔自己的 metadata 都寫
`n_speakers = 1`；官方 config 裡那個 256 是**嵌入表的大小**，不是聲音的數量。所以每個模型
就是一個聲音，表裡另外 255 格是空的——**這正是為什麼填一個沒訓練過的編號會算出無聲而不是報錯**。
3.4.0 給了一個數字輸入格然後在後面接住那個無聲；現在不給了。工具那邊仍然收 `speaker=`（給
真的有多個聲音的模型用，也是無聲拒絕能講出編號的原因），只是面板不再問。

### 3.4.0 有一條規則被推翻了

3.4.0 讓「預設值拼成空字串」，這樣沒調過音的句子可以沿用既有的快取檔。省下來的是真的，
**但記錄的東西是錯的**：空字串記的不是「這些數字」，而是「這個版本認為的預設值」。所以上面那個
0.667 → 0.6 一改，每一個用空字串命名的檔案就會開始宣稱自己是一個它並不包含的聲音——同一個
專案裡，一句是快取的、一句是新算的，聽起來不一樣，畫面上什麼都不會說。

**現在預設值一律完整拼出來。** 代價是每個離線句子會再算一次，一句四秒的 CPU、零元——這個區別
正是它在這裡付得起、在雲端供應商那裡付不起的原因。

## 3.4.0 - 2026-08-14

**離線模型的聲音可以自己調了。**

模型裡面本來就不只一個聲音。`island_chatter_local` 卻把四個數字寫死在程式裡，所以一顆裝著好幾個
語者的模型，只講得出其中一個。現在選著離線來源的時候，「金鑰」那顆按鈕會變成**「調音…」**，
打開一個有四格的視窗：

- **語者**：挑模型裡的哪一個聲音。留白就是模型自己講的那個——**留白不等於 0**，中文模型自己宣告
  的是 1，把兩者當成同一件事，等於幫所有沒開過這個視窗的人換了聲音。
- **變化**（`noise_scale`）和**音色變化**（`noise_scale_w`）：0 到 2，越大每次唸出來的差異越大。
- **語速**：0.25 到 4，**越大越快**。

### 語速這一格是改變主意，理由寫在這裡

它本來被釘在 1，理由是「面板上已經有一個 Speed，兩個做不同事的語速控制比一個糟」。那個理由是錯的：
面板的 Speed 是**內建合成器**的，而離線語音是一個匯入的 WAV，那條線上的引擎是靜音的。
**Speed 根本碰不到離線模型唸出來的句子**——所以那條線本來就沒有語速可調。

模型自己的參數叫 `length_scale`，它乘的是「長度」，所以方向跟一般人講的「語速」是**相反**的。
反轉發生在填張量的那一行，不在對話框裡：一個數字越大唸得越慢的「語速」欄位，沒有人用得下去。
`ae-local-verify.jsx` 會實際唸兩次，要求 0.5 唸出來的**比較長**。

### 調音會進快取鍵，而且是靠「它就是那個 voice」

四個數字每一個都會改變聲音，所以每一個都**必須**進快取檔名——否則改了設定按下去，回來的是上一次
設定的音檔，畫面上沒有任何東西會說為什麼。它們走的是 `voice` 欄位，因為那個欄位早就在快取鍵裡了，
而且對一個跑在本機的來源來說它本來沒有別的意思（供應商表填的是字面上的 `"default"`）。

**加第八個欄位是第一版的做法，被換掉的理由值得記下來：** 那會改掉**每一筆**快取的雜湊，包含三家
雲端供應商的——升級之後第一次按下去，就會重抓一句已經付過錢的話。8ab 說購買不能藏在一個按鍵後面，
它也不能藏在一次更新後面。同樣的道理，**預設值會拼成空字串**，所以 3.3.0 之前算好的離線句子，
檔名一個都沒有變。

工具那邊讀完會**正規化**寫回去，所以 `0.8` 和 `0.800` 是同一筆快取而不是兩個內容相同的檔案；
名字不認得的設定是**直接拒絕**，不是忽略——被安靜丟掉的設定，會唸出一個沒有人要求的聲音，
然後回報成功。

面板和 `cloud.hpp` 各有一份範圍和名字。`npm test` 逐項比對，還拿 `island_chatter_local --help`
裡那行範例去比對面板拼出來的字串——分隔符或欄位名任何一邊改掉，都會在這裡壞掉，
而不是在使用者第一次按下去的時候。
