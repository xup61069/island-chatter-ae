...
**面板顯示版本號，試用版的標記更明顯，試用版一次限十層。**

- **版本號**在語言選單旁邊，例如 `3.9.0`，試用版是 `3.9.0 試用版`。它是**跟工具問來的**，
  不是寫在面板裡：面板是純文字、跟旁邊那些工具分開安裝，寫在面板裡的版本號會是「面板的版本」，
  第一次有人手動複製單一檔案就過期了。`npm test` 會擋下任何寫死在面板裡、長得像版本號的字串。
- **標記聲：0.10 → 0.30，每四秒 → 每兩秒，第一次 1.5 秒 → 0.8 秒。**
- **試用版一次最多套用十層。**

### 舊的標記其實大部分句子都沒有蓋到

第一次在 1.5 秒、之後每 4 秒——**而一句台詞就是一兩秒**。所以試用版裡多數句子完全沒有標記，
浮水印只蓋到長句子。測試現在會**數**前 1.5 秒裡有幾個標記，那是只看音量的檢查做不到的判斷。

音量本來是「怕吵到買家」寫出來的數字。真正該怕的是反過來：**小聲到可以混在對白下面的標記，
就是有人會直接拿去用的標記。**

### 圖層限制推翻了 8ai 原本的論點

8ai 寫得很清楚：圖層限制會讓人沒辦法判斷二十句話擺在一起成不成立。那句話沒有錯，只是被推翻了。

而且它是**軟限制**，文件裡就這樣寫：面板是純文字，會用 Notepad 的人都能把 10 改掉。它是路障。
**真正的強制是那個標記聲，因為它編譯在引擎裡。**

### 大聲的標記找出引擎裡一個真的 bug

`apply_output_gain()` 在增益剛好等於 1.0 時會提早 return，理由是「乘以一不會改變任何東西」。
乘以一確實不會——但那個函式**還會做限幅**，提早 return 等於連限幅一起跳過；而 `copy_region()`
沒有這個捷徑。所以兩條輸出路徑對**所有超過拐點的取樣**都不一致，**而且是在預設設定下**——
預設音量的 `output_gain()` 剛好就是 1.0。也就是說：**轉出來的音檔可能跟 After Effects 播的不一樣。**

它一直躲著，是因為要有取樣超過 `kSoftKnee` 才看得到，安靜的素材永遠碰不到。0.30 的標記一下就
碰到了，測試回報「同一個音色逐區塊算出來不一樣」。**標記不是那個 bug，是它讓 bug 大聲到看得見。**

另外兩個測試本來是**透過**浮水印去量合成器的（長音的延續、唱出來的音高），現在會把標記減掉或
跳過——因為標記是取樣索引的純函數，所以那是精確扣除而不是近似，正式版扣的是零。
