...
- 讀取 AE 目前選取的文字圖層
- 可辨識的 CV 音節：爆破音、摩擦音、鼻音、流音，以及 `a/e/i/o/u` 共振峰
...
Island Chatter 會將文字組合成 CV（子音＋母音）音節。子音使用爆破、摩擦、齒擦、鼻音、流音或送氣起音；母音則以不同的聲道共振峰合成 `a/e/i/o/u`。英文的相鄰子母音會合併，例如 `ba`、`si`；中文等文字會依字碼穩定產生不同的擬音音節。相同文字和設定會得到一致的節奏與音色，標點符號會產生自然停頓。
...
這會在專案根目錄建立被 Git 忽略的 `preview.wav`。
